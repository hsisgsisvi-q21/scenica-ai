          ...
          {DATA.map((inf, i) => (
            <div key={inf.name} className="inf-profile cursor-pointer" data-aos="fade-up" data-aos-delay={i * 80} onClick={() => setSel(sel === inf.name ? null : inf.name)}>
              <div className="aspect-[16/9] relative overflow-hidden">
                <img src={YT(inf.vids[0])} onError={(e)=>{e.target.src=YT_FB(inf.vids[0])}} alt="" className="w-full h-full object-cover" style={{ objectPosition: '50% 20%' }} />
              ...
              {sel === inf.name && (
                <div className="p-5 md:p-6 border-t border-white/[.03]">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {inf.vids.map((v, j) => (
                      <div key={j} className="aspect-video rounded overflow-hidden"><img src={YT(v)} onError={(e)=>{e.target.src=YT_FB(v)}} alt="" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700 pointer-events-none" /></div>
                    ))}
                  </div>
